...
variable "ddm_api_host" {
  description = "The DDM API URL. Ensure that this URL is reachable from the machine running terraform. For example, if the API URL shown in DDM is `ddm.private.address/graphql`, but the local device can only reach your DDM's public address, you can instead set this to `ddm.public.address/graphql`."
  type        = string
}
