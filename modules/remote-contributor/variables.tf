...
variable "firewall_tags" {
  description = "A list of firewall tags the Remote Contributor instance will be associated with."
  type        = set(string)
}
...
variable "component_name_abbreviation" {
  description = "(Optional) The abbreviated name used for the given Remote Contributor."
  type        = string
  default     = "rc"
  nullable    = false
}
...
variable "resource_url" {
  description = "(Optional) The url to download a remote-contributor installer"
  type        = string
  nullable    = false
...
variable "create_lb" {
  description = <<-EOT
  (Optional) If true, a load balancer in front of the Remote Contributor (to perform HTTPS termination) and required
...
variable "license_key" {
  description = "(Optional) The Remote Contributor license provided by Audinate"
  type        = string
  sensitive   = true
  nullable    = true
  default     = null
...
variable "timeout_sec" {
  description = "(Optional) Timeout of the request from the load balancer to the Remote Contributor. Default is 30 seconds. Valid range is [1, 86400]."
  type        = number
  default     = null
}
